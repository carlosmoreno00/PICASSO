...
class BackendController extends Controller
{
    function main() {
        ...
        $last5comments = DB::select('SELECT comments.iduser, users.name as nickname, photos.photo as photo, comments.created_at, comments.description FROM users, photos, comments where  photos.iduser = users.id and photos.id = comments.idphoto ORDER BY created_at DESC LIMIT 5');
        $usersWithMorePhotos = DB::select('select users.name, count(*) as totalphotos from photos, users where photos.iduser = users.id GROUP BY users.name ORDER by totalphotos desc limit 5');
